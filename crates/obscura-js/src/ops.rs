...
pub fn realm_state(scope: &mut v8::PinScope<'_, '_>, op_state: &OpState) -> SharedState {
    let page = || op_state.borrow::<SharedState>().clone();
...
#[op2]
#[string]
fn op_get_cookies(scope: &mut v8::PinScope<'_, '_>, state: &OpState) -> String {
...
#[op2(fast)]
fn op_set_cookie(scope: &mut v8::PinScope<'_, '_>, state: &OpState, #[string] cookie_str: &str) {
    let gs = realm_state(scope, state);
    let gs = gs.borrow();
    let jar = match &gs.cookie_jar {
        Some(j) => j,
